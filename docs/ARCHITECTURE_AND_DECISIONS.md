...
**Execution.** Tool calls run the extension's compiled `tools/<name>.js` in a fresh, short-lived Bun worker process (`src/worker-executor.ts`), with a shimmed `@raycast/api` module injected via the module cache (`src/shims.ts:651-708`). Extensions are loaded and shimmed, never rewritten. OAuth tokens and preferences are read directly from Raycast's encrypted SQLite database using the macOS Keychain key plus Raycast's salt (`src/auth.ts:33-46`), so existing Raycast logins work without re-authentication.
...
7. **Tool executes** against the shimmed `@raycast/api`.
   *Failure (gated API):* `Raycast API <name> is disabled in RayBridge. Enable raycastApi.<flag> ...` (`src/shims.ts:207-213`). *Failure (auth):* expired/missing token → error matching the auth regex.
...
- [ ] `docs/CONFIGURATION.md` — tools.json modes, all six `raycastApi` gates, env vars, preferences.json (README config section; `src/config.ts`, `src/shims.ts:25-59`)
