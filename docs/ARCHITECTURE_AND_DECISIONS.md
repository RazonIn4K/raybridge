...
| # | Unknown | Recommended answer | Reasoning | Implications | Code ref |
|---|---------|--------------------|-----------|--------------|----------|
| H1 | Why were React/OpenTUI in dependencies, and can they be removed? | Resolved: moved to `optionalDependencies` and lazy-loaded. They power only the interactive config TUI (`raybridge config`), which the server never imports | `src/tui.tsx` is the sole consumer of `react`/`@opentui/react`/`@opentui/core`; `src/cli.ts` dynamic-imports the TUI for the `config` command only and falls back to install guidance when the packages are absent (`isMissingTuiDependency`) | Core server and tool execution have no hard dep on them; default `bun install` still installs optional deps so typecheck stays green, only `--omit=optional` drops the TUI. Chosen over fully retiring the TUI (would remove a working feature) | `src/tui.tsx:3-6`, `src/cli.ts` (`config` case + `isMissingTuiDependency`), `package.json` `optionalDependencies` |
