...
async function main() {
  ...
  assert(base64, "Expected the base64 Raycast extension to be installed");

  const tool = base64.tools.find((entry) => entry.name === "base64-encode");
  assert(tool, "Expected base64/base64-encode to be installed");
  ...
  await executeToolInWorker(
    ...
    .catch((err: any) => {
      assert(
        String(err?.message ?? err).includes("fixture failure"),
  ...
  try {
    await executeToolInWorker(
      ...
      .catch((err: any) => {
        assert(
          String(err?.message ?? err).includes("timed out"),
